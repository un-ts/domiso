...
const incorrectOverrides: Partial<Record<string, string>> = {
  // https://github.com/cure53/DOMPurify/issues/886
  ...
  // https://github.com/cure53/DOMPurify/issues/889
  166: `<textarea type="text" cols="50" rows="10"></textarea>//["'\`--&gt;]]&gt;]`,
}
...
describe('dompurify compatibility', () => {
  ...
  for (const [index, { title, payload, expected }] of Object.entries(
    ...
    test(name, () => {
      const result = sanitize(payload, true).trim()
      const expects =
